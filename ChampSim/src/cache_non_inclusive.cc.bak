...
bool CACHE::manage_dirty(uint32_t set, uint32_t way, int level){
    ...
    if (block[set][way].dirty) {
        ...
        if (lower_level) {
            if (lower_level->get_occupancy(2, block[set][way].address) == lower_level->get_size(2, block[set][way].address)) {
                ...
                writeback_packet.fill_level = fill_level << level;
...
void CACHE::back_invalidate(int cache_level, uint64_t addr, uint32_t set, uint32_t way){
    bool dirty_managed = false;
    if (block[set][way].valid == 1){ // victim is valid
        if (cache_level == IS_LLC){
            uint32_t l2c_set = upper_level_dcache[0]->get_set(addr);
            uint32_t l2c_way = upper_level_dcache[0]->get_way(addr, l2c_set);
            if (l2c_way != L2C_WAY){
                uint32_t l1d_set = upper_level_dcache[0]->upper_level_dcache[0]->get_set(addr);
                uint32_t l1d_way = upper_level_dcache[0]->upper_level_dcache[0]->get_way(addr, l1d_set);
                if (l1d_way != L1D_WAY){
                    dirty_managed = upper_level_dcache[0]->upper_level_dcache[0]->manage_dirty(set, way, 3);
                    upper_level_dcache[0]->upper_level_dcache[0]->invalidate_entry(addr);
                }
                if (!dirty_managed) dirty_managed = upper_level_dcache[0]->manage_dirty(set, way, 2);
                upper_level_dcache[0]->invalidate_entry(addr);
            }
            if (!dirty_managed) manage_dirty(set, way, 1);
            invalidate_entry(addr);
        }
        else if (cache_level == IS_L2C){
            uint32_t l1d_set = upper_level_dcache[0]->get_set(addr);
            uint32_t l1d_way = upper_level_dcache[0]->get_way(addr, l1d_set);
            if (l1d_way != L1D_WAY){
                dirty_managed = upper_level_dcache[0]->manage_dirty(set, way, 2);
                upper_level_dcache[0]->invalidate_entry(addr);
            }
            if (!dirty_managed) manage_dirty(set, way, 1);
            invalidate_entry(addr);
