...
os.system("mkdir -p data_set1")

for inclu in inclusivity:
    os.system(f"mkdir -p data_set1/{inclu}")
    os.system(f"cp src/cache_{inclu}.cc.bak src/cache.cc")

    for f in fac11:

        os.system(f"mkdir -p data_set1/{inclu}/{f}")
        ...
        line_number = 12
        lines[line_number] = f'#define fac11 {f}\n'
        ...
        os.system(f"mv results_{N_SIM}M/* data_set1/{inclu}/{f}/")
