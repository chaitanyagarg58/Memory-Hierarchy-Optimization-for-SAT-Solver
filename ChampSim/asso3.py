...
fac1 = ["0.0625", "0.25", "0.5", "1", "2"]

os.system("mkdir -p data_asso3")

for inclu in inclusivity:
    os.system(f"mkdir -p data_asso3/{inclu}")
    os.system("cp src/cache_" + inclu + ".cc.bak src/cache.cc")

    for f in fac1:

        os.system(f"mkdir -p data_asso3/{inclu}/{f}")
        ...
        # Modify the particular line that needs to be changed
        line_number = 10
        lines[line_number] = f'# define fac3 {f}\n'
        ...
        os.system(f"mv results_{N_SIM}M/* data_asso3/{inclu}/{f}/")
