...
os.system("mkdir -p data_set3")

for inclu in inclusivity:
    os.system(f"mkdir -p data_set3/{inclu}")
    os.system(f"cp src/cache_{inclu}.cc.bak src/cache.cc")

    for f in fac33:

        os.system(f"mkdir -p data_set3/{inclu}/{f}")
        ...
        line_number = 14
        lines[line_number] = f'#define fac33 {f}\n'
        ...
        os.system(f"mv results_{N_SIM}M/* data_set3/{inclu}/{f}/")
