...
fac2 = ["0.125", "0.25", "0.5", "1", "2"]

os.system("mkdir -p data_asso2")

for inclu in inclusivity:
    os.system(f"mkdir -p data_asso2/{inclu}")
    os.system("cp src/cache_" + inclu + ".cc.bak src/cache.cc")

    for f in fac2:

        os.system(f"mkdir -p data_asso2/{inclu}/{f}")
        ...
        # Modify the particular line that needs to be changed
        line_number = 9
        lines[line_number] = f'# define fac2 {f}\n'
        ...
        os.system(f"mv results_{N_SIM}M/* data_asso2/{inclu}/{f}/")
