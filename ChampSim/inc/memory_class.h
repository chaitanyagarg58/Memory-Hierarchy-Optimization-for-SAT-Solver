...
class MEMORY {
  public:
    ...
    virtual int invalidate_entry(uint64_t inval_addr){return 0;};
    virtual uint32_t get_set(uint64_t address){return 0;};
    virtual uint32_t get_way(uint64_t address, uint32_t set){return 0;};
    virtual bool manage_dirty(uint32_t set, uint32_t way, int level){return true;};
